...
#define AW_MONITOR_NAME "aw_monitor.bin"
				       ...
				       unsigned int *vol)
{
	...
	ret = aw_dev->ops.aw_reg_read(aw_dev, desc->reg, (u16 *)vol);
	if (ret < 0) {
...
static int aw_monitor_get_battery_state(struct aw_device *aw_dev, int *data,
					int data_type)
{
	char name[] = "battery";
	int ret = -1;
...
static int aw_monitor_get_system_temperature(struct aw_device *aw_dev,
					     int *temp)
{
	int ret = -1;
	int sys_temp = 0;

	ret = aw_monitor_get_battery_state(aw_dev, &sys_temp,
...
static int aw_monitor_set_system_temperature(struct aw_device *aw_dev,
					     int sys_temp)
{
	int ret = -1;
	struct aw_dsp_monitor_desc *dsp_monitor_desc =
		&aw_dev->monitor_desc.dsp_monitor_desc;

	ret = aw_dev->ops.aw_dsp_write(aw_dev,
					dsp_monitor_desc->extn_temp_reg,
					sys_temp,
					dsp_monitor_desc->extn_temp_reg_type);
	if (ret < 0) {
					 ...
					 unsigned int *vol)
{
	...
	ret = aw_monitor_get_battery_state(aw_dev, &sys_vol,
	...
	if (monitor->first_entry == AW_FIRST_ENTRY) {
		...
		if (monitor->samp_count == 0) {
			aw_dev_err(aw_dev->dev,
				   "monitor->samp_count:%d unsupported",
				   monitor->samp_count);
			return -EINVAL;
		}
...
static void aw_monitor_set_ipeak(struct aw_device *aw_dev, u16 ipeak)
{
...
static void aw_monitor_set_gain(struct aw_device *aw_dev, u16 gain)
{
	u32 monitor_vol = 0;
	u32 compared_vol = 0;
	int ret = -1;
...
static void aw_monitor_set_vmax(struct aw_device *aw_dev, u32 vmax)
{
	struct aw_monitor_cfg *monitor_cfg = &aw_dev->monitor_desc.monitor_cfg;
	...
	vmax_set = (u16)((int16_t)vmax + (int16_t)desc->init_vmax);

	ret = aw_dev->ops.aw_dsp_write(aw_dev, desc->dsp_reg, vmax_set,
				       desc->data_type);
	if (ret < 0)
		return;
...
static void aw_monitor_check_sysint(struct aw_device *aw_dev)
{
	...
	ret = aw_device_params(aw_dev, AW_DEV_INT_PARAMS, (void *)&sysint,
			       sizeof(sysint), AW_GET_DEV_PARAMS);
	if (ret < 0) {
	...
	if (sysint & (desc->intst_mask)) {
		...
		aw_dev_err(aw_dev->dev, "check sysint fail, reg=0x%04x",
			   sysint);
	}
}
...
static int aw_monitor_work(struct aw_device *aw_dev)
{
	...
	if (aw_cali_get_cali_status(&aw_dev->cali_desc)) {
...
static void aw_sw_monitor_work_func(struct work_struct *work)
{
	...
	if ((monitor_cfg->monitor_status == AW_MON_CFG_OK) &&
		...
		if (hmute_st == AW_DEV_HMUTE_DISABLE) {
			...
			queue_delayed_work(aw->work_queue, &monitor->delay_work,
...
static void aw_dsp_monitor_work_func(struct work_struct *work)
{
	...
	int ret = -1;
	struct aw_device *aw_dev = container_of(work, struct aw_device,
						monitor_desc.dsp_monitor_work.work);
	struct aw *aw = (struct aw *)aw_dev->private_data;
	struct aw_monitor_desc *monitor = &aw_dev->monitor_desc;
	...
	if (hmute_st == AW_DEV_HMUTE_DISABLE) {
		...
		queue_delayed_work(aw->work_queue, &monitor->dsp_monitor_work,
...
{
	struct aw_device *aw_dev = container_of(monitor_desc, struct aw_device,
						monitor_desc);
	struct aw *aw = (struct aw *)aw_dev->private_data;
	...
	if (monitor_desc->dsp_temp_flag == AW_EXTERNAL_TEMP) {
		...
		aw_dev_dbg(aw_dev->dev,
			   "Internal chip temperature used");
	}
}
...
static void aw_sw_monitor_start(struct aw_monitor_desc *monitor_desc)
{
	...
	queue_delayed_work(aw->work_queue, &monitor_desc->delay_work, 0);
}
...
static void aw_monitor_get_hw_monitor_st(struct aw_monitor_desc *monitor_desc)
{
	...
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "read 0x%02x failed",
			   hw_monitor_desc->reg);
		return;
	}
...
{
	int ret = -1;
	u32 temp_reg_value = 0;
	u32 volt_reg_value = 0;
	u32 dsp_volt_en = 0;
	u32 dsp_temp_en = 0;
	u32 extern_temp_en = 0;
	struct aw_device *aw_dev = container_of(monitor_desc, struct aw_device,
						monitor_desc);
	struct aw_dsp_monitor_desc *dsp_monitor_desc =
		&monitor_desc->dsp_monitor_desc;
	...
	ret = aw_dev->ops.aw_dsp_read(aw_dev,
				       dsp_monitor_desc->dsp_temp_reg,
				       &temp_reg_value,
				       dsp_monitor_desc->dsp_temp_reg_type);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "read 0x%04x failed",
			   dsp_monitor_desc->dsp_temp_reg);
		return;
	}

	ret = aw_dev->ops.aw_dsp_read(aw_dev,
				       dsp_monitor_desc->dsp_volt_reg,
				       &volt_reg_value,
				       dsp_monitor_desc->dsp_volt_reg_type);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "read 0x%04x failed",
			   dsp_monitor_desc->dsp_temp_reg);
		return;
	...
	extern_temp_en = temp_reg_value &
			 (~dsp_monitor_desc->extn_temp_en_mask);
...
static void aw_monitor_set_init_vmax(struct aw_monitor_desc *monitor_desc)
{
	...
	ret = aw_dev->ops.aw_dsp_write(aw_dev, desc->dsp_reg, desc->init_vmax,
				       desc->data_type);
	if (ret < 0) {
...
static void aw_monitor_get_init_vmax(struct aw_monitor_desc *monitor_desc)
{
	...
	ret = aw_dev->ops.aw_dsp_read(aw_dev, desc->dsp_reg, &vmax,
				      desc->data_type);
	if (ret < 0) {
	...
	desc->init_vmax = vmax;
	aw_dev_info(aw_dev->dev, "get init vmax:0x%x", desc->init_vmax);
}
 ...
 * load monitor config
 *****************************************************/
static int aw_monitor_param_check_sum(struct aw_device *aw_dev, u8 *data,
				       u32 data_len)
{
	int i = 0, check_sum = 0;
					   ...
					   const char *offset_ptr)
{
	...
	for (i = 0; i < (int)(table_info->table_num * table_size);
	     i += table_size) {
...
static int aw_monitor_parse_vol_data_v_0_1_1(struct aw_device *aw_dev, u8 *data)
{
	struct aw_monitor_hdr_v_0_1_1 *monitor_hdr =
		(struct aw_monitor_hdr_v_0_1_1 *)data;
...
static int aw_monitor_parse_temp_data_v_0_1_1(struct aw_device *aw_dev, u8 *data)
{
	struct aw_monitor_hdr_v_0_1_1 *monitor_hdr =
		(struct aw_monitor_hdr_v_0_1_1 *)data;
...
static void aw_monitor_parse_hdr_v_0_1_1(struct aw_device *aw_dev, u8 *data)
{
	struct aw_monitor_hdr_v_0_1_1 *monitor_hdr =
		(struct aw_monitor_hdr_v_0_1_1 *)data;
	struct aw_monitor_cfg *monitor_cfg = &aw_dev->monitor_desc.monitor_cfg;
	...
	monitor_cfg->monitor_switch =
		(monitor_hdr->enable_flag >> MONITOR_EN_BIT) &
		MONITOR_EN_MASK;
...
static int aw_monitor_check_fw_v_0_1_1(struct aw_device *aw_dev, u8 *data,
				       u32 data_len)
{
	struct aw_monitor_hdr_v_0_1_1 *monitor_hdr =
		(struct aw_monitor_hdr_v_0_1_1 *)data;
	...
	temp_size = (u32)(monitor_hdr->temp_num *
			  monitor_hdr->single_temp_size);
	if (temp_size > data_len) {
		aw_dev_err(aw_dev->dev,
			   "temp_size:[%d] overflow file size[%d]!",
			   temp_size, data_len);
		return -ENOMEM;
	}

	vol_size = (u32)(monitor_hdr->vol_num *
			 monitor_hdr->single_vol_size);
	if (vol_size > data_len) {
		aw_dev_err(aw_dev->dev,
			   "vol_size:[%d] overflow file size[%d]!",
			   vol_size, data_len);
		return -ENOMEM;
	}
...
static int aw_monitor_parse_data_v_0_1_1(struct aw_device *aw_dev, u8 *data,
					 u32 data_len)
{
	int ret = -1;
	struct aw_monitor_cfg *monitor_cfg = &aw_dev->monitor_desc.monitor_cfg;
	...
	if (ret < 0) {
		if (monitor_cfg->temp_info.aw_table) {
...
static void aw_monitor_free_firmware(struct aw_device *aw_dev)
{
	...
	if (monitor_cfg->vol_info.aw_table) {
...
static int aw_monitor_real_time_update_monitor(struct aw_device *aw_dev)
{
	...
	ret = request_firmware(&cont, AW_MONITOR_NAME, aw_dev->dev);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "failed to read %s", AW_MONITOR_NAME);
		release_firmware(cont);
		return ret;
	}

	aw_monitor_cnt = devm_kzalloc(aw_dev->dev,
				       cont->size + sizeof(u32), GFP_KERNEL);
	if (!aw_monitor_cnt) {
		release_firmware(cont);
		return -ENOMEM;
	}
			 ...
			 char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "aw temp: %d\n",
			    ...
			    char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "aw monitor_flag=%u\n",
...
static void aw_monitor_parse_dt(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		...
		aw_dev_info(aw_dev->dev, "parse hw-monitor-delay:[%d]",
			    monitor_delay);
...
void aw_monitor_init(struct aw_monitor_desc *monitor_desc)
{
	...
	INIT_DELAYED_WORK(&monitor_desc->dsp_monitor_work,
			  aw_dsp_monitor_work_func);
