...
/* aw_bin_parse.c aw codec driver
 *
...
static char *profile_name[AW_PROFILE_MAX] = {"Music", "Voice", "Voip",
	"Ringtone", "Ringtone_hs", "Lowpower", "Bypass", "Mmi", "Fm",
...
int aw_dev_dsp_data_order(struct aw_device *aw_dev, u8 *data, u32 data_len)
{
	int i = 0;
	u8 tmp_val = 0;
...
static int aw_dev_parse_raw_reg(struct aw_device *aw_dev, u8 *data,
				u32 data_len, struct aw_prof_desc *prof_desc)
{
...
static int aw_dev_parse_raw_dsp_cfg(struct aw_device *aw_dev, u8 *data,
				    u32 data_len, struct aw_prof_desc *prof_desc)
{
	int ret;
...
static int aw_dev_parse_raw_dsp_fw(struct aw_device *aw_dev, u8 *data,
				   u32 data_len, struct aw_prof_desc *prof_desc)
{
	int ret;
...
static int aw_dev_prof_parse_multi_bin(struct aw_device *aw_dev, u8 *data,
				       u32 data_len,
				       struct aw_prof_desc *prof_desc)
{
	...
	for (i = 0; i < aw_bin->all_bin_parse_num; i++) {
		...
		} else if (aw_bin->header_info[i].bin_data_type ==
			   DATA_TYPE_DSP_REG) {
			ret = aw_dev_dsp_data_order(aw_dev,
				data + aw_bin->header_info[i].valid_data_addr,
				aw_bin->header_info[i].valid_data_len);
			if (ret < 0)
				return ret;
					 ...
					 struct aw_prof_desc *scene_prof_desc)
{
	...
	case ACF_SEC_TYPE_MUTLBIN:
		return aw_dev_prof_parse_multi_bin(aw_dev,
			...
			cfg_dde->data_size, scene_prof_desc);
	default:
		aw_dev_err(aw_dev->dev, "unsupported type:%u",
			   cfg_dde->data_type);
		return -EINVAL;
	}
	return 0;
		...
		if ((aw_dev->i2c->adapter->nr == cfg_dde[i].dev_bus) &&
			...
			if (cfg_dde[i].data_type == ACF_SEC_TYPE_MONITOR) {
				ret = aw_monitor_parse_fw(&aw_dev->monitor_desc,
					(u8 *)prof_hdr + cfg_dde[i].data_offset,
					cfg_dde[i].data_size);
				if (ret < 0) {
					   ...
					   struct aw_cfg_hdr *prof_hdr)
{
	...
	for (i = 0; i < prof_hdr->a_ddt_num; i++) {
		if ((aw_dev->i2c->adapter->nr == cfg_dde[i].dev_bus) &&
		    (aw_dev->i2c->addr == cfg_dde[i].dev_addr) &&
		    (aw_dev->chip_id == cfg_dde[i].chip_id)) {
			ret = aw_dev_parse_data_by_sec_type_v_1_0_0_0(aw_dev,
					    ...
					    struct aw_container *aw_cfg)
{
	...
	ret = aw_dev_parse_scene_count_v_1_0_0_0(aw_dev, aw_cfg,
						 &prof_info->count);
	if (ret < 0) {
...
static u8 aw_dev_crc8_check(unsigned char *data, u32 data_size)
{
	u8 crc_value = 0x00;
	u8 pdatabuf = 0;
	int i;
...
static int aw_dev_check_cfg_by_hdr(struct aw_container *aw_cfg)
{
	...
	for (i = 0; i < cfg_hdr->a_ddt_num; i++) {
		...
		/* crc check */
		act_crc8 = aw_dev_crc8_check(aw_cfg->data +
