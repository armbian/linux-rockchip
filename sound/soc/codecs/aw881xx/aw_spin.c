...
static const char *const aw_spin[] = {"spin_0", "spin_90", "spin_180",
				       "spin_270"};

#ifdef AW_MTK_PLATFORM_SPIN
extern int mtk_spk_send_ipi_buf_to_dsp(void *data_buffer, u32 data_size);
...
extern int aw_send_afe_cal_apr(u32 param_id, void *buf, int cmd_size,
			       bool write);
#else
static int aw_send_afe_cal_apr(u32 param_id, void *buf, int cmd_size,
			       bool write)
{
	return 0;
}
...
static int aw_get_msg_id(int dev_ch, u32 *msg_id)
{
	switch (dev_ch) {
...
static int aw_mtk_set_spin_angle(struct aw_device *aw_dev, u32 spin_angle)
{
	int ret = -1;

	ret = aw_mtk_write_data_to_dsp(AW_MSG_ID_SPIN, &spin_angle,
				       sizeof(u32));
	if (ret)
...
static int aw_mtk_set_mixer_en(struct aw_device *aw_dev, u32 msg_id,
			       int32_t is_enable)
{
	...
	dsp_msg = kzalloc(sizeof(struct aw_msg_hdr) + sizeof(int32_t),
			  GFP_KERNEL);
	if (!dsp_msg)
		return -ENOMEM;
...
static int aw_qcom_write_data_to_dsp(u32 msg_id, void *data, int size)
{
	int ret = -1;
...
static int aw_qcom_read_data_from_dsp(u32 msg_id, void *data, int size)
{
	int ret = -1;
...
static int aw_qcom_set_spin_angle(struct aw_device *aw_dev, u32 spin_angle)
{
	int ret = -1;

	ret = aw_qcom_write_data_to_dsp(AW_MSG_ID_SPIN, &spin_angle,
					sizeof(u32));
	if (ret) {
		aw_dev_err(aw_dev->dev, "write spin angle to dsp failed");
	} else {
		aw_dev_info(aw_dev->dev,
			    "write spin angle to dsp successful");
	}

	return ret;
}

static int aw_qcom_get_spin_angle(u32 *spin_angle, int size)
{
	int ret = -1;
...
static int aw_qcom_set_mixer_en(struct aw_device *aw_dev, u32 msg_id,
				int32_t is_enable)
{
	int32_t *dsp_msg = NULL;
	int ret = 0;
...
/***********************************spin_angle**********************************/
static int aw_set_adsp_spin_angle(struct aw_device *aw_dev, u32 spin_angle)
{
...
static void aw_get_adsp_spin_angle(u32 *spin_angle)
{
#ifdef AW_MTK_PLATFORM_SPIN
	aw_mtk_get_spin_angle(spin_angle, sizeof(u32));
#else
	aw_qcom_get_spin_angle(spin_angle, sizeof(u32));
#endif
}
/*******************************************************************************/
...
static int aw_hold_reg_spin_st(struct aw_spin_desc *spin_desc)
{
	struct aw_device *aw_dev = container_of(spin_desc, struct aw_device,
						spin_desc);
	u16 reg_val = 0;
	...
	if ((g_spin_mode == AW_REG_SPIN_MODE) ||
		...
		aw_dev->ops.aw_reg_read(aw_dev,
					 aw_dev->chansel_desc.rxchan_reg,
					 &reg_val);
...
int aw_spin_check_mode(struct aw_spin_desc *spin_desc)
{
	...
	aw->spin_flag = AW_SPIN_ON;

	return 0;
}

static int aw_hold_dsp_spin_st(struct aw_spin_desc *spin_desc)
{
	struct aw_device *aw_dev = container_of(spin_desc, struct aw_device,
						spin_desc);
	int ret = -1;
	...
	if (aw_dev->channel == 0) {
		if (g_spin_mode == AW_ADSP_SPIN_MODE) {
			ret = aw_set_adsp_spin_angle(aw_dev, g_spin_angle);
			if (ret < 0)
				return ret;
		}
	}
...
int aw_spin_hold_chan_params(struct aw_spin_desc *spin_desc, u8 type)
{
	...
	switch (type) {
	case AW_REG_SPIN_ST:
		return aw_hold_reg_spin_st(spin_desc);
	case AW_DSP_SPIN_ST:
		return aw_hold_dsp_spin_st(spin_desc);
	default:
...
static int aw_set_reg_spin_angle(struct aw *aw, u32 spin_angle)
{
	struct list_head *pos = NULL;
	struct list_head *dev_list = NULL;
	struct aw_device *local_pa = NULL;
	...
	if (spin_angle >= ARRAY_SIZE(aw_spin)) {
		aw_dev_err(aw->dev, "spin_angle:%d not support", spin_angle);
		return -EINVAL;
	}

	ret = aw_dev_get_list_head(&dev_list);
	if (ret) {
		aw_dev_err(aw->dev, "get dev list failed");
		return ret;
	}

	list_for_each(pos, dev_list) {
		local_pa = container_of(pos, struct aw_device, list_node);
		ret = aw_device_params(local_pa, AW_DEV_CHANNEL_MODE_PARAMS,
				       (void *)&spin_angle, sizeof(spin_angle),
				       AW_SET_DEV_PARAMS);
		if (ret < 0) {
...
static int aw_set_reg_mixer_spin_angle(struct aw *aw, u32 spin_angle)
{
	int ret = -1;
	...
	ret = aw_set_mixer_en(aw->aw_pa, AW_AUDIO_MIX_ENABLE);
	if (ret)
		return ret;

	usleep_range(AW_100000_US, AW_100000_US + 10);

	aw_set_reg_spin_angle(aw, spin_angle);

	ret = aw_set_mixer_en(aw->aw_pa, AW_AUDIO_MIX_DISABLE);
	if (ret)
		return ret;
...
static void aw_get_reg_spin_angle(u32 *spin_angle)
{
...
static int aw_set_spin_angle(struct aw *aw, u32 spin_angle)
{
	switch (g_spin_mode) {
	case AW_REG_SPIN_MODE:
		return aw_set_reg_spin_angle(aw, spin_angle);
	case AW_ADSP_SPIN_MODE:
		return aw_set_adsp_spin_angle(aw->aw_pa, spin_angle);
	case AW_REG_MIXER_SPIN_MODE:
		return aw_set_reg_mixer_spin_angle(aw, spin_angle);
	default:
...
{
	struct aw *aw = (struct aw *)kcontrol->private_value;
	u32 ctrl_value = 0;
	int ret = -1;

	aw_dev_dbg(aw->dev, "ucontrol->value.integer.value[0]=%ld",
		   ucontrol->value.integer.value[0]);

	if (aw->spin_flag == AW_SPIN_OFF) {
		aw_dev_dbg(aw->dev, "spin func not enable");
		return 0;
	}

	ctrl_value = (u32)ucontrol->value.integer.value[0];

	mutex_lock(&g_aw_spin_lock);
	if (aw->pstream == AW_STREAM_OPEN) {
		ret = aw_set_spin_angle(aw, ctrl_value);
		if (ret < 0)
			aw_dev_err(aw->dev, "set spin error, ret=%d\n", ret);
	} else {
		if ((g_spin_mode == AW_REG_SPIN_MODE) ||
		    (g_spin_mode == AW_REG_MIXER_SPIN_MODE)) {
			aw_set_reg_spin_angle(aw, ctrl_value);
		} else {
			aw_dev_info(aw->dev,
...
static void aw_get_spin_angle(u32 *spin_angle)
{
	if ((g_spin_mode == AW_REG_SPIN_MODE) ||
	    (g_spin_mode == AW_REG_MIXER_SPIN_MODE)) {
		       ...
		       struct snd_ctl_elem_value *ucontrol)
{
	...
	if (aw->pstream == AW_STREAM_OPEN) {
		...
		aw_dev_dbg(aw->dev, "no stream, use record value");
	}
	mutex_unlock(&g_aw_spin_lock);
			...
			struct snd_ctl_elem_info *uinfo)
{
	...
	if (!aw) {
		aw_pr_err("get struct aw failed");
		return -EINVAL;
	}
...
static int aw_spin_control_create(struct aw *aw)
{
	int kcontrol_num = 1;
	struct snd_kcontrol_new *aw_spin_control = NULL;
	char *kctl_name = NULL;

	aw_spin_control = devm_kzalloc(aw->codec->dev,
					sizeof(struct snd_kcontrol_new) * 1,
					GFP_KERNEL);
	if (!aw_spin_control)
		return -ENOMEM;

	kctl_name = devm_kzalloc(aw->codec->dev, AW_NAME_BUF_MAX,
				  GFP_KERNEL);
	if (!kctl_name)
		return -ENOMEM;
	...
	aw_spin_control[0].private_value = (unsigned long)aw;

	aw->codec_ops->add_codec_controls(aw->codec, aw_spin_control,
					   kcontrol_num);
...
{
	struct aw *aw = (struct aw *)aw_dev;

	if (aw->aw_pa->spin_desc.spin_mode != AW_SPIN_OFF_MODE)
		aw_spin_control_create(aw);
}
				   ...
				   struct device_node *np)
{
	...
	for (i = 0; i < AW_SPIN_MAX; i++) {
		if (spin_table_str[i] == 'l' || spin_table_str[i] == 'L') {
			aw_dev->spin_desc.spin_table[i].rx_val =
	...
	if (spin_count != ARRAY_SIZE(aw_spin)) {
		aw_dev_err(aw_dev->dev,
			   "get spin_data failed, spin_count:%d", spin_count);
		return -EINVAL;
	}
...
static void aw_parse_port_id_dt(struct aw_device *aw_dev)
{
	...
	aw_dev_info(aw_dev->dev, "rx-port-id: 0x%x", g_rx_port_id);
}
