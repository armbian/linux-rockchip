...
#define PKA_RAM_FOR_PKA()	do { \
	if ((PKA_READ(RKCE_PKA_RAM_CTL_OFFSET) & 0x03) == RKCE_PKA_RAM_CTL_PKA_RDY) \
		break; \
	if (PKA_READ(RKCE_PKA_RAM_ST_OFFSET) & 0x01) \
		PKA_WRITE(RKCE_PKA_RAM_ST_CLK_RAM_RDY_MASK, RKCE_PKA_RAM_ST_OFFSET); \
	PKA_WRITE((RKCE_PKA_RAM_CTL_PKA_RDY << RKCE_WRITE_MASK_SHIFT) | \
		RKCE_PKA_RAM_CTL_PKA_RDY, RKCE_PKA_RAM_CTL_OFFSET); \
	while ((PKA_READ(RKCE_PKA_RAM_ST_OFFSET) & 0x01) != RKCE_PKA_RAM_ST_CLK_RAM_RDY_MASK) \
		cpu_relax(); \
	PKA_WRITE(RKCE_PKA_RAM_ST_CLK_RAM_RDY_MASK, RKCE_PKA_RAM_ST_OFFSET);\
} while (0)

#define PKA_RAM_FOR_CPU()	do { \
	if ((PKA_READ(RKCE_PKA_RAM_CTL_OFFSET) & 0x03) == 0) \
		break; \
	if (PKA_READ(RKCE_PKA_RAM_ST_OFFSET) & 0x01) \
		PKA_WRITE(RKCE_PKA_RAM_ST_CLK_RAM_RDY_MASK, RKCE_PKA_RAM_ST_OFFSET); \
	PKA_WRITE((RKCE_PKA_RAM_CTL_PKA_RDY << RKCE_WRITE_MASK_SHIFT), \
			RKCE_PKA_RAM_CTL_OFFSET); \
	while ((PKA_READ(RKCE_PKA_RAM_ST_OFFSET) & 0x01) != RKCE_PKA_RAM_ST_CLK_RAM_RDY_MASK) \
		cpu_relax(); \
	PKA_WRITE(RKCE_PKA_RAM_ST_CLK_RAM_RDY_MASK, RKCE_PKA_RAM_ST_OFFSET);\
} while (0)
