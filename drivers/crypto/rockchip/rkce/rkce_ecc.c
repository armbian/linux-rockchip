...
uint32_t rkce_ecp_max_bits = 512;
 ...
 */
static const uint32_t secp384r1_wide = RK_ECC_CURVE_WIDE_384;
...
static int rk_ecp_group_load(struct rk_ecp_group *grp, enum rk_ecp_group_id id)
{
	...
	case RK_ECP_DP_SECP384R1:
		RK_LOAD_GROUP_A(secp384r1);
		return 0;
			...
			struct rkce_ecp_point *point_P, struct rkce_ecp_point *point_sign)
{
	...
	bn_hash = rkce_bn_alloc(RKCE_ECP_MAX_BYTES);
	if (!bn_hash) {
		ret = -ENOMEM;
		goto exit;
	}
...
exit:
	if (ret || rk_word_cmp_zero(ecc_st->v, RKCE_ECP_MAX_WORDS)) {
		ret = -EKEYREJECTED;
		dump_ecc_sram();
	}
...
void rkce_ecc_init(void __iomem *base)
{
	...
	rkce_ecp_max_bits = RK_ECP_READ_REG(RK_ECC_MAX_CURVE_WIDE);
