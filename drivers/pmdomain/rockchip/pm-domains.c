...
#define DOMAIN_M_O_R_G(_name, p_offset, pwr, status, m_offset, m_status, r_status, r_offset, req, idle, ack, g_mask, delay, wakeup, keepon)	\
{							\
	.name = _name,					\
	.pwr_offset = p_offset,				\
	.pwr_w_mask = (pwr) << 16,			\
...
#define DOMAIN_RK3576(name, p_offset, pwr, status, r_status, r_offset, req, idle, g_mask, delay, wakeup)	\
	DOMAIN_M_O_R_G(name, p_offset, pwr, status, 0, r_status, r_status, r_offset, req, idle, idle, g_mask, delay, wakeup, false)

#define DOMAIN_RK3576_P(name, p_offset, pwr, status, r_status, r_offset, req, idle, g_mask, delay, wakeup)	\
	DOMAIN_M_O_R_G(name, p_offset, pwr, status, 0, r_status, r_status, r_offset, req, idle, idle, g_mask, delay, wakeup, true)
					 ...
					 bool idle)
{
	...
	if (IS_ENABLED(CONFIG_ROCKCHIP_FPGA))
		return 0;
				      ...
				      struct device_node *node)
{
	...
	if (pd->num_qos > 0) {
		...
		for (j = 1; j < MAX_QOS_REGS_NUM; j++)
			pd->qos_is_need_init[j] = pd->qos_is_need_init[j - 1] + num_qos;
	...
	if (IS_ENABLED(CONFIG_ROCKCHIP_FPGA) && (pd->genpd.flags & GENPD_FLAG_ALWAYS_ON))
...
static const struct rockchip_domain_info rk3572_pm_domains[] = {
	...
	[RK3572_PD_IMG]		= DOMAIN_RK3572("img",       0x0, BIT(9),   0,      BIT(7),  0x0, BIT(11), 0, 0,       false),
