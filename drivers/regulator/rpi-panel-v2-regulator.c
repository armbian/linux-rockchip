...
static int rpi_panel_v2_gpio_get_direction(struct gpio_chip *gc,
					   unsigned int off)
{
	return GPIO_LINE_DIRECTION_OUT;
}

static void rpi_panel_v2_gpio_set(struct gpio_chip *gc, unsigned int off,
				  int val)
{
	...
	poweron_state = state->poweron_state;
	if (val)
		poweron_state |= BIT(off);
	else
		poweron_state &= ~BIT(off);

	state->poweron_state = poweron_state;
	regmap_write(state->regmap, REG_POWERON, poweron_state);
...
static int rpi_panel_v2_i2c_read(struct i2c_client *client, u8 reg,
				 unsigned int *buf)
{
