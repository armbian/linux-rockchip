...
#define DW9807_NAME			"dw9807"
  ...
  *
  * Adaptive step size for smoother movement:
  * - Small distance (≤32): use 4 steps for finer control
  * - Medium distance (≤128): use 8 steps
  * - Large distance (>128): use 16 steps
  *
 ...
 {
     int step, val, ret;
     int distance, ctrl_steps;

     /* Calculate distance and choose appropriate step size */
     distance = abs(end - start);
     if (distance <= 32)
         ctrl_steps = 4;   /* Fine control for small movements */
     else if (distance <= 128)
         ctrl_steps = 8;   /* Medium steps */
     else
         ctrl_steps = 16;  /* Larger steps for long distances */
     ...
     while (true) {
         ...
         /* Skip busy wait for smoother continuous movement */
...
static int dw9807_parse_dt(struct i2c_client *client,
			  struct dw9807_device *dw9807_dev)
{
 ...
 static int dw9807_probe(struct i2c_client *client)
 {
     ...
     /* Read register 0x00 to check device presence */
     {
         s32 ret;
         u8 reg_data;
         ...
         if (ret < 0)
             dev_info(&client->dev, "probe: I2C read reg 0x00 failed ret=%d\n", ret);
         else {
             reg_data = (u8)ret;
             dev_info(&client->dev, "probe: reg 0x00 = 0x%02x (device detected)\n",
                  reg_data);
         }
     }
     ...
     dw9807_parse_dt(client, dw9807_dev);
     ...
     /* Set subdev name following dw9714 convention: m%02d_%s_%s %s */
     {
         ...
         if (strcmp(dw9807_dev->module_facing, "back") == 0)
             facing[0] = 'b';
         else
             facing[0] = 'f';

         snprintf(dw9807_dev->sd.name, sizeof(dw9807_dev->sd.name),
                  "m%02d_%s_%s %s",
                  dw9807_dev->module_index, facing,
                  DW9807_NAME, dev_name(&client->dev));
     }
