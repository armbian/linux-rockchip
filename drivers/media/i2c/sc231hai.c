				  ...
				  struct v4l2_mbus_config *config)
{
	...
	config->bus.mipi_csi2.num_data_lanes = SC231HAI_LANES;
...
static long sc231hai_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_SET_SYNC_MODE:
		...
		if (sync_mode) {
			...
			dev_info(&sc231hai->client->dev, "set sync mode is: %s\n",
				 ((*sync_mode == EXTERNAL_MASTER_MODE) ||
				  (*sync_mode == SLAVE_MODE)) ? "secondary" : "primary");
		} else {
			dev_info(&sc231hai->client->dev, "set sync mode is: NO_SYNC_MODE\n");
		}
		break;
...
static void sc231hai_remove(struct i2c_client *client)
{
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
	struct sc231hai *sc231hai = to_sc231hai(sd);
