...
static const struct regval ox03c10_1920x1080_30fps_linear_lcg_raw10_mipi996[] = {
	{0x4221, 0x05},
	{0x5002, 0x3f},
	{0x5003, 0x2a},
	{0x502c, 0x0f},
...
static const struct regval ox03c10_1920x1080_30fps_linear_hcg_raw10_mipi996[] = {
	{0x4221, 0x05},
	{0x5002, 0x3f},
	{0x5003, 0x2a},
	{0x502c, 0x0f},
...
static const struct regval ox03c10_1920x1080_30fps_linear_spd_raw10_mipi996[] = {
	{0x4221, 0x05},
	{0x5002, 0x3f},
	{0x5003, 0x2a},
	{0x502c, 0x0f},
...
static const struct ox03c10_mode supported_modes[] = {
	{
		...
		.linear_reg_list = ox03c10_1920x1080_30fps_linear_lcg_raw10_mipi996,
		.hdr_mode = NO_HDR,
		.hdr_compr = NULL,
		.bpp = 10,
		.mipi_freq_idx = 1,
	...
	},
	{
		...
		.vts_def = 0x02ae * 2,
		.reg_list = ox03c10_1920x1080_30fps_HDR3_LFM_PWL16_mipi996,
		.linear_reg_list = ox03c10_1920x1080_30fps_linear_hcg_raw10_mipi996,
		.hdr_mode = NO_HDR,
		.hdr_compr = NULL,
		.bpp = 10,
		.mipi_freq_idx = 1,
	...
	},
	{
		...
		.vts_def = 0x02ae * 2,
		.reg_list = ox03c10_1920x1080_30fps_HDR3_LFM_PWL16_mipi996,
		.linear_reg_list = ox03c10_1920x1080_30fps_linear_vs_raw10_mipi996,
		.hdr_mode = NO_HDR,
		.hdr_compr = NULL,
		.bpp = 10,
		.mipi_freq_idx = 1,
	...
	},
	{
		...
		.vts_def = 0x02ae * 2,
		.reg_list = ox03c10_1920x1080_30fps_HDR3_LFM_PWL16_mipi996,
		.linear_reg_list = ox03c10_1920x1080_30fps_linear_spd_raw10_mipi996,
		.hdr_mode = NO_HDR,
		.hdr_compr = NULL,
		.bpp = 10,
		.mipi_freq_idx = 1,
...
static int ox03c10_select_expand_single_mode(struct ox03c10 *ox03c10, u32 single_mode)
{
	int ret = -EINVAL;
	u32 i, h, w, hdr_mode;
				  ...
				  unsigned int cmd, unsigned long arg)
{
	...
	case RKMODULE_SET_CMPS_MODE:
		...
		break;
	case RKMODULE_SET_EXPAND_SINGLE_MODE:
		if (copy_from_user(&single_mode, up, sizeof(u32)))
			return -EFAULT;
...
static int __ox03c10_start_stream(struct ox03c10 *ox03c10)
{
	...
	if (ox03c10->cur_mode->hdr_mode == NO_HDR) {
		if (ox03c10->cur_mode->single_mode == EXPAND_SINGLE_HCG)
			dev_info(&ox03c10->client->dev, "%s mode: LINEAR_HCG\n", __func__);
		else if (ox03c10->cur_mode->single_mode == EXPAND_SINGLE_LCG)
			dev_info(&ox03c10->client->dev, "%s mode: LINEAR_LCG\n", __func__);
		else if (ox03c10->cur_mode->single_mode == EXPAND_SINGLE_VS)
			dev_info(&ox03c10->client->dev, "%s mode: LINEAR_VS\n", __func__);
		else if (ox03c10->cur_mode->single_mode == EXPAND_SINGLE_SPD)
			dev_info(&ox03c10->client->dev, "%s mode: LINEAR_SPD\n", __func__);
	} else {
		if (ox03c10->cur_mode->hdr_operating_mode == OX03C10_HDR3_DCG_VS_12BIT)
			dev_info(&ox03c10->client->dev, "%s mode: HDR3_DCG_VS_12BIT\n", __func__);
		else if (ox03c10->cur_mode->hdr_operating_mode == OX03C10_HDR3_DCG_SPD_12BIT)
			dev_info(&ox03c10->client->dev, "%s mode: HDR3_DCG_SPD_12BIT\n", __func__);
		else if (ox03c10->cur_mode->hdr_operating_mode == OX03C10_HDR3_DCG_VS_LFM_16BIT)
			dev_info(&ox03c10->client->dev, "%s mode: HDR3_DCG_VS_16BIT\n", __func__);
	}
	...
	if (ox03c10->cur_mode->hdr_mode == NO_HDR) {
		ret = ox03c10_write_array(ox03c10->client, ox03c10->cur_mode->linear_reg_list);
		if (ret)
			return ret;
	}
...
static int ox03c10_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_EXPOSURE:
		...
		if (ox03c10->streaming)
			ret |= ox03c10_write_reg(ox03c10->client, OX03C10_GROUP_UPDATE_ADDRESS,
						 OX03C10_REG_VALUE_08BIT,
						 OX03C10_GROUP_UPDATE_START_DATA);
		if (ox03c10->cur_mode->single_mode == EXPAND_SINGLE_LCG ||
		    ox03c10->cur_mode->single_mode == EXPAND_SINGLE_HCG) {
			exp_reg = OX03C10_REG_EXPOSURE_DCG_H;
		} else if (ox03c10->cur_mode->single_mode == EXPAND_SINGLE_VS) {
			exp_reg = OX03C10_REG_EXPOSURE_VS_H;
		} else if (ox03c10->cur_mode->single_mode == EXPAND_SINGLE_SPD) {
			exp_reg = OX03C10_REG_EXPOSURE_SPD_H;
		} else {
			dev_err(&client->dev, "%s cur single mode not support, pls check it",
				__func__);
			break;
		}
		ret |= ox03c10_write_reg(ox03c10->client,
	...
	case V4L2_CID_ANALOGUE_GAIN:
		...
		*/
		...
		} else if (ox03c10->cur_mode->single_mode == EXPAND_SINGLE_HCG) {
			again_reg = OX03C10_REG_AGAIN_HCG_H;
			dgain_reg = OX03C10_REG_DGAIN_HCG_H;
		} else if (ox03c10->cur_mode->single_mode == EXPAND_SINGLE_VS) {
			again_reg = OX03C10_REG_AGAIN_VS_H;
			dgain_reg = OX03C10_REG_DGAIN_VS_H;
		} else if (ox03c10->cur_mode->single_mode == EXPAND_SINGLE_SPD) {
			again_reg = OX03C10_REG_AGAIN_SPD_H;
			dgain_reg = OX03C10_REG_DGAIN_SPD_H;
		} else {
			dev_err(&client->dev, "%s cur single mode not support, pls check it",
				__func__);
			break;
