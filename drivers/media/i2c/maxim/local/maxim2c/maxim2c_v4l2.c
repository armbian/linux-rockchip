...
static int maxim2c_support_mode_init(maxim2c_t *maxim2c)
{
	...
	if ((array_size > 0) &&
			((array_size * sizeof(u32)) % sizeof(struct maxim2c_vc_info) == 0) &&
			((array_size * sizeof(u32)) <= sizeof(struct maxim2c_vc_info) * PAD_MAX)) {
...
#endif
#if KERNEL_VERSION(6, 12, 0) <= LINUX_VERSION_CODE
	.get_frame_interval = maxim2c_g_frame_interval,
#endif
};
