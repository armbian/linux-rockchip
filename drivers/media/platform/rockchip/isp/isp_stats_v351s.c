...
rkisp_stats_get_sharp_stats(struct rkisp_isp_stats_vdev *stats_vdev,
			    struct rkisp351s_stat_buffer *pbuf)
{
	struct rkisp_device *dev = stats_vdev->dev;
	struct rkisp_isp_params_vdev *params = &dev->params_vdev;
	struct isp351s_isp_params_cfg *params_rec = params->isp351s_params + dev->unite_index;
...
rkisp_stats_get_bay3d_stats(struct rkisp_isp_stats_vdev *stats_vdev,
			    struct rkisp351s_stat_buffer *pbuf)
{
	struct rkisp_isp_params_val_v351s *priv = stats_vdev->dev->params_vdev.priv_val;
	...
	if (val & 0x1) {
		...
		pbuf->stat.buf_aipre_index = priv->aipre_cur_idx;
	}
	return 0;
}
			   ...
			   struct rkisp351s_stat_buffer *pbuf)
{
	struct rkisp_device *dev = stats_vdev->dev;
	struct rkisp_isp_params_vdev *params = &dev->params_vdev;
	struct rkisp_isp_params_val_v351s *priv_val = params->priv_val;
	struct isp351s_isp_params_cfg *params_rec = params->isp351s_params + dev->unite_index;
	...
	if (val & 0x1) {
		...
		if (pbuf) {
			hist = &pbuf->stat.hist;
			memcpy(hist->iir_data, arg_rec->iir_data, sizeof(hist->iir_data));
			  ...
			  struct rkisp351s_stat_buffer *pbuf)
{
	...
	if (val & 0x1) {
		...
		while (timeout--) {
			...
			if (val & ISP351S_ENH_IIR_RDATA_VAL)
				break;
			udelay(2);
		}
		if (timeout < 0) {
			v4l2_warn(&dev->v4l2_dev, "%s enh read timeout\n", __func__);
			return 0;
		}
		for (i = 0; i < num; i++)
			arg_rec->iir_data[i] = isp3_stats_read(stats_vdev, ISP33_ENH_IIR0);
		if (dev->is_frm_rd)
			arg_rec->iir_wr = true;
		if (pbuf) {
			enh = &pbuf->stat.enh;
			memcpy(enh->iir_data, arg_rec->iir_data, sizeof(enh->iir_data));
			    ...
			    struct rkisp351s_stat_buffer *pbuf)
{
	...
	struct rkisp_isp_params_val_v351s *priv_val = params_vdev->priv_val;
...
{
	struct rkisp_device *dev = stats_vdev->dev;
	struct rkisp_isp_params_val_v351s *priv_val;
...
rkisp_stats_send_meas_fe(struct rkisp_isp_stats_vdev *stats_vdev, u32 w3a_ris)
{
	...
	struct rkisp_device *dev = stats_vdev->dev;
	struct rkisp351s_stat_buffer *stat_tmp_buf, *stat_buf = NULL;
	...
	if (cur_buf) {
		...
		stat_buf->stat.buf_aipre_index = -1;
...
rkisp_stats_send_meas(struct rkisp_isp_stats_vdev *stats_vdev, u32 w3a_ris)
{
	...
	struct rkisp351s_stat_buffer *stat_tmp_buf = NULL, *cur_stat_buf = NULL;
	u32 cur_frame_id, size = stats_vdev->vdev_fmt.fmt.meta.buffersize;
	...
	if (cur_stat_buf) {
		...
		cur_stat_buf->stat.buf_aipre_index = -1;
...
static void
rkisp_stats_isr_v351s(struct rkisp_isp_stats_vdev *stats_vdev,
...
static void
rkisp_get_stat_size_v351s(struct rkisp_isp_stats_vdev *stats_vdev,
			  unsigned int sizes[])
{
	int mult = stats_vdev->dev->unite_div;

	sizes[0] = ALIGN(sizeof(struct rkisp351s_stat_buffer), 16);
	sizes[0] *= mult;
	stats_vdev->vdev_fmt.fmt.meta.buffersize = sizes[0];
}

static void
rkisp_stats_first_ddr_config_v351s(struct rkisp_isp_stats_vdev *stats_vdev)
{
	struct rkisp_device *dev = stats_vdev->dev;
	...
	rkisp_get_stat_size_v351s(stats_vdev, &size);
...
static void
rkisp_stats_next_ddr_config_v351s(struct rkisp_isp_stats_vdev *stats_vdev)
{
	struct rkisp_device *dev = stats_vdev->dev;
...
static void rkisp_stats_stop_v351s(struct rkisp_isp_stats_vdev *stats_vdev)
{
	struct rkisp_device *dev = stats_vdev->dev;
	u32 val, addr;
...
static struct rkisp_isp_stats_ops rkisp_isp_stats_ops_tbl = {
	.isr_hdl = rkisp_stats_isr_v351s,
	.get_stat_size = rkisp_get_stat_size_v351s,
	.first_ddr_cfg = rkisp_stats_first_ddr_config_v351s,
	.next_ddr_cfg = rkisp_stats_next_ddr_config_v351s,
	.stats_stop = rkisp_stats_stop_v351s,
};
