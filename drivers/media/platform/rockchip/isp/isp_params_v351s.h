/* SPDX-License-Identifier: GPL-2.0 */
/* Copyright (c) 2025 Rockchip Electronics Co., Ltd. */

#ifndef _RKISP_ISP_PARAM_V351S_H
#define _RKISP_ISP_PARAM_V351S_H

#include "isp_params.h"

#define ISP351S_RAWHISTBIG_ROW_NUM		15
#define ISP351S_RAWHISTBIG_COLUMN_NUM		15
#define ISP351S_RAWHISTBIG_WEIGHT_REG_SIZE	\
	(ISP351S_RAWHISTBIG_ROW_NUM * ISP351S_RAWHISTBIG_COLUMN_NUM)

#define ISP351S_HIST_THUMB_ROW_MAX 16
#define ISP351S_HIST_THUMB_COL_MAX 20

struct rkisp_isp_params_vdev;
struct rkisp_isp_params_val_v351s {
	struct rkisp_dummy_buffer buf_ldch[ISP_UNITE_MAX][ISP3X_MESH_BUF_NUM];
	u32 buf_ldch_idx[ISP_UNITE_MAX];
	u32 ldch_out_hsize;
	struct rkisp_dummy_buffer buf_ldcv[ISP_UNITE_MAX][ISP3X_MESH_BUF_NUM];
	u32 buf_ldcv_idx[ISP_UNITE_MAX];
	u32 ldcv_out_vsize;

	struct rkisp_dummy_buffer buf_b3dldc[ISP_UNITE_MAX][ISP3X_MESH_BUF_NUM];
	u32 buf_b3dldc_idx[ISP_UNITE_MAX];
	u32 b3dldc_hsize;
	u32 b3dldch_vsize;
	u32 b3dldcv_vsize;

	struct rkisp_dummy_buffer buf_info[RKISP_INFO2DDR_BUF_MAX];
	u32 buf_info_owner;
	u32 buf_info_cnt;
	u32 buf_info_w_offs;
	u32 buf_info_v_offs;
	int buf_info_idx;

	struct rkisp_dummy_buffer buf_aiawb[RKISP_BUFFER_MAX];
	u32 buf_aiawb_cnt;
	int buf_aiawb_idx;

	struct rkisp_dummy_buffer buf_bay3d_wgt[RKISP_BUFFER_MAX];
	struct rkisp_dummy_buffer buf_bay3d_iir[RKISP_BUFFER_MAX];
	struct rkisp_dummy_buffer buf_bay3d_ds[RKISP_BUFFER_MAX];
	struct rkisp_dummy_buffer buf_gain[RKISP_BUFFER_MAX];
	struct rkisp_dummy_buffer buf_aipre[RKISP_BUFFER_MAX];
	struct rkisp_dummy_buffer buf_aiisp[RKISP_BUFFER_MAX];

	struct rkisp_dummy_buffer buf_bay3d_iir_l2;
	struct rkisp_dummy_buffer buf_bay3d_wgt_l2;
	struct rkisp_dummy_buffer buf_bay3d_ds_l2;

	spinlock_t buf_lock;
	struct list_head iir_list;
	struct list_head gain_list;
	struct list_head aipre_list;
	struct rkisp_dummy_buffer *pbuf_bay3d_iir;
	struct rkisp_dummy_buffer *pbuf_gain_wr;
	struct rkisp_dummy_buffer *pbuf_gain_rd;
	struct rkisp_dummy_buffer *pbuf_aipre;
	struct rkisp_dummy_buffer *pbuf_aiisp;

	u32 bay3d_iir_rw_fmt;
	u32 bay3d_iir_pk_offs[ISP_UNITE_MAX];
	u32 bay3d_iir_pk_stride[ISP_UNITE_MAX];
	u32 bay3d_iir_stride[ISP_UNITE_MAX];
	u32 bay3d_iir_size[ISP_UNITE_MAX];
	u32 bay3d_iir_offs[ISP_UNITE_MAX];
	int bay3d_iir_cnt;
	int bay3d_iir_idx;
	int bay3d_iir_cur_idx;

	u32 bay3d_ds_size[ISP_UNITE_MAX];
	u32 bay3d_ds_offs[ISP_UNITE_MAX];
	int bay3d_ds_cnt;
	int bay3d_ds_idx;
	int bay3d_ds_cur_idx;

	u32 bay3d_wgt_size[ISP_UNITE_MAX];
	u32 bay3d_wgt_offs[ISP_UNITE_MAX];
	int bay3d_wgt_cnt;
	int bay3d_wgt_idx;
	int bay3d_wgt_cur_idx;

	int aiisp_cnt;
	int aiisp_cur_idx;

	u32 gain_size;
	int gain_cnt;
	int gain_cur_idx;

	u32 aipre_stride;
	int aipre_cnt;
	int aipre_idx;
	int aipre_cur_idx;

	u32 hist_blk_num[ISP_UNITE_MAX];
	u32 enh_scl_16bit_num[ISP_UNITE_MAX];

	bool is_ae0_fe;
	bool is_ae3_fe;
	bool is_af_fe;
	bool is_awb_fe;
	bool is_aiawb_fe;
	bool is_aiisp_l2_buf;
};

#if IS_ENABLED(CONFIG_VIDEO_ROCKCHIP_ISP_VERSION_V35_1)
int rkisp_init_params_vdev_v351s(struct rkisp_isp_params_vdev *params_vdev);
void rkisp_uninit_params_vdev_v351s(struct rkisp_isp_params_vdev *params_vdev);
#else
static inline int rkisp_init_params_vdev_v351s(struct rkisp_isp_params_vdev *params_vdev) { return -EINVAL; }
static inline void rkisp_uninit_params_vdev_v351s(struct rkisp_isp_params_vdev *params_vdev) {}
#endif

#endif /* _RKISP_ISP_PARAM_V351S_H */
