...
static void calc_unite_scl_params(struct rkvpss_stream *stream)
{
	...
	bool use_average = (stream->id == RKVPSS_OUTPUT_CH2)
			   && stream->avg_scl_down;
 ...
 */
static int rkvpss_get_zme_coe_sel(struct rkvpss_device *dev, u32 factor, bool scale_up)
{
	int coe_sel;
	...
	if (scale_up) {
		/* Scale up: factor = (src - 1) * 2^16 / (dst - 1), range: ~25k to ~43k */
		if (factor <= 24572)
			coe_sel = 0;
		...
		v4l2_dbg(5, rkvpss_debug, &dev->v4l2_dev,
			 "%s: scale_up coe_sel=%d (factor=%u, thresholds: 24572/32768/43690)\n",
			 __func__, coe_sel, factor);
	} else {
		/* Scale down: factor = (src - 1) * 2^12 / (dst - 1), range: ~4.9k to ~16k */
		if (factor <= 4917)
			coe_sel = 4;
		else if (factor <= 5851)
			coe_sel = 5;
...
/*
 * v2.1 polyphase scale for channel 0
 * Uses ZME module with fixed 10 coefficient sets selectable via register bits
 */
static void polyphase_scale(struct rkvpss_stream *stream, bool on, bool sync)
{
	struct rkvpss_device *dev = stream->dev;
	u32 out_w = stream->out_fmt.width;
	u32 out_h = stream->out_fmt.height;
	...
	int h_coe_sel, v_coe_sel;
	bool h_scale_up, v_scale_up;

	v4l2_dbg(2, rkvpss_debug, &dev->v4l2_dev,
		 "%s: stream:%d on:%d sync:%d in(%dx%d) out(%dx%d)\n",
		 __func__, stream->id, on, sync, in_w, in_h, out_w, out_h);

	if (!on) {
		rkvpss_unite_write(dev, RKVPSS2X_ZME_Y_SCL_CTRL, 0);
		rkvpss_unite_write(dev, RKVPSS2X_ZME_UV_SCL_CTRL, 0);
		/* Update ZME shadow registers when disabling */
		rkvpss_unite_write(dev, RKVPSS2X_ZME_UPDATE,
				   RKVPSS_SCL_GEN_UPD | RKVPSS_SCL_FORCE_UPD);
		v4l2_dbg(3, rkvpss_debug, &dev->v4l2_dev,
			 "%s: stream:%d ZME disabled\n", __func__, stream->id);
		return;
	}

	/* constraints: check scaling ratio limits */
	...
	rkvpss_unite_write(dev, RKVPSS2X_ZME_Y_SRC_SIZE, val);
	rkvpss_unite_write(dev, RKVPSS2X_ZME_UV_SRC_SIZE, val);

	val = (out_w - 1) | ((out_h - 1) << 16);
	rkvpss_unite_write(dev, RKVPSS2X_ZME_Y_DST_SIZE, val);
	rkvpss_unite_write(dev, RKVPSS2X_ZME_UV_DST_SIZE, val);
	...
	if (in_w != out_w) {
		...
		rkvpss_unite_write(dev, RKVPSS2X_ZME_Y_XSCL_FAC, y_xscl_fac);
		rkvpss_unite_write(dev, RKVPSS2X_ZME_UV_XSCL_FAC, uv_xscl_fac);

		/* Select horizontal filter coefficient */
		h_coe_sel = rkvpss_get_zme_coe_sel(dev, y_xscl_fac, h_scale_up);
	...
	if (in_h != out_h) {
		...
		rkvpss_unite_write(dev, RKVPSS2X_ZME_Y_YSCL_FAC, y_yscl_fac);
		rkvpss_unite_write(dev, RKVPSS2X_ZME_UV_YSCL_FAC, uv_yscl_fac);

		/* Select vertical filter coefficient */
		v_coe_sel = rkvpss_get_zme_coe_sel(dev, y_yscl_fac, v_scale_up);
		y_ctrl |= RKVPSS2X_ZME_TAP6_COE_SEL(v_coe_sel);
		uv_ctrl |= RKVPSS2X_ZME_TAP6_COE_SEL(v_coe_sel);
	}

	/* Write control registers */
	rkvpss_unite_write(dev, RKVPSS2X_ZME_Y_SCL_CTRL, y_ctrl);
	rkvpss_unite_write(dev, RKVPSS2X_ZME_UV_SCL_CTRL, uv_ctrl);

	/* Update ZME shadow registers (0x1804, NOT 0x1004!) */
	val = RKVPSS_SCL_GEN_UPD;
	if (sync)
		val |= RKVPSS_SCL_FORCE_UPD;
	rkvpss_unite_write(dev, RKVPSS2X_ZME_UPDATE, val);

	v4l2_dbg(3, rkvpss_debug, &dev->v4l2_dev,
		 "CH0 polyphase: in(%dx%d) out(%dx%d) h_coe:%d v_coe:%d\n",
		 in_w, in_h, out_w, out_h,
		 (in_w != out_w) ? h_coe_sel : -1,
		 (in_h != out_h) ? v_coe_sel : -1);
	v4l2_dbg(3, rkvpss_debug, &dev->v4l2_dev,
		 "ZME regs: y_ctrl=0x%x uv_ctrl=0x%x y_xfac=%u y_yfac=%u\n",
		 y_ctrl, uv_ctrl,
		 (in_w != out_w) ? y_xscl_fac : 0,
		 (in_h != out_h) ? y_yscl_fac : 0);
}

/* v2.1: sharpen configuration for CH0~CH2 (3-select-1) */
void rkvpss_sharp_config_v21(struct rkvpss_device *dev,
			      struct rkvpss_sharp_cfg *cfg, bool sync)
{
	u32 ctrl, val;

	if (!cfg || !cfg->enable) {
		/* Disable sharpen */
		rkvpss_unite_write(dev, RKVPSS2X_SHARP_CTRL, 0);
		val = RKVPSS2X_SHARP_FORCE_UPD;
		rkvpss_unite_write(dev, RKVPSS2X_SHARP_UPDATE, val);
		return;
	}
	...
	val = RKVPSS2X_SHARP_FACTOR(cfg->factor) |
	      RKVPSS2X_SHARP_THRESHOLD(cfg->threshold);
	rkvpss_unite_write(dev, RKVPSS2X_SHARP_SHARPEN, val);

	/* Configure control: enable and channel select */
	ctrl = RKVPSS2X_SHARP_EN | RKVPSS2X_SHARP_SEL(cfg->channel);
	rkvpss_unite_write(dev, RKVPSS2X_SHARP_CTRL, ctrl);
	...
	v4l2_dbg(2, rkvpss_debug, &dev->v4l2_dev,
		 ...
		 cfg->channel, cfg->factor, cfg->threshold);
}
...
static int rkvpss_stream_scale(struct rkvpss_stream *stream, bool on, bool sync)
{
	bool use_avg_scale = (stream->id == 2) && stream->avg_scl_down;

	if (stream->avg_scl_down && !use_avg_scale)
		v4l2_warn(&stream->dev->v4l2_dev,
			 "CH%d: average_scale_down not supported (only CH2)\n", stream->id);

	if (stream->id == RKVPSS_OUTPUT_CH0) {
		/* v2.1: CH0 only uses polyphase scaling */
		v4l2_dbg(2, rkvpss_debug, &stream->dev->v4l2_dev,
			"CH%d: polyphase_scale, on:%d sync:%d\n", stream->id, on, sync);
		polyphase_scale(stream, on, sync);
			       ...
			       int unite_index)
{
	...
	if (ctrl) {
		...
		if (!cfg->disable_mosaic_avg_en)
			ctrl |= RKVPSS2X_CMSC_MOSAIC_AVG_EN;  /* v2.1: enable mosaic average sampling */
		v4l2_dbg(2, rkvpss_debug, &dev->v4l2_dev,
			"cmsc_config_hw: unite:%d index:%d ctrl=0x%x mosaic_block=%d mosaic_avg_en=%d\n",
			dev->unite_mode, unite_index, ctrl, cfg->mosaic_block,
			!cfg->disable_mosaic_avg_en);
	}
...
static int rkvpss_set_cmsc(struct rkvpss_stream *stream, struct rkvpss_cmsc_cfg *cfg)
{
	...
	dev->cmsc_cfg.disable_mosaic_avg_en = cfg->disable_mosaic_avg_en;
	dev->cmsc_upd = true;
	v4l2_dbg(3, rkvpss_debug, &dev->v4l2_dev,
		 "%s ch:%d win_en:0x%x mosaic_block=%d mosaic_avg_en=%d\n",
		 __func__, stream->id,
		 dev->cmsc_cfg.win[stream->id].win_en,
		 cfg->mosaic_block, !cfg->disable_mosaic_avg_en);
...
static int rkvpss_get_sharp(struct rkvpss_stream *stream, struct rkvpss_sharp_cfg *sharp)
{
	struct rkvpss_device *vpss_dev = stream->dev;

	*sharp = vpss_dev->sharp_cfg;
	return 0;
}

static int rkvpss_set_sharp(struct rkvpss_stream *stream, struct rkvpss_sharp_cfg *sharp)
{
	struct rkvpss_device *vpss_dev = stream->dev;

	/* Validate channel selection (0~2) */
	if (sharp->channel > 2) {
		v4l2_warn(&vpss_dev->v4l2_dev,
			  "sharpen: invalid channel %d\n", sharp->channel);
		return -EINVAL;
	}

	vpss_dev->sharp_cfg = *sharp;

	/* Apply configuration immediately if streaming */
	if (stream->streaming)
		rkvpss_sharp_config_v21(vpss_dev, &vpss_dev->sharp_cfg, false);
				 ...
				 bool valid_prio, unsigned int cmd, void *arg)
{
	...
	case RKVPSS_CMD_SET_AVG_SCL_DOWN:
		...
		break;
	case RKVPSS_CMD_GET_SHARP:
		ret = rkvpss_get_sharp(stream, arg);
		break;
	case RKVPSS_CMD_SET_SHARP:
		ret = rkvpss_set_sharp(stream, arg);
		break;
	default:
		ret = -EINVAL;
	}
